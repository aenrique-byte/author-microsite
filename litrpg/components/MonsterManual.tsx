...
import { Ghost, Loader2, Plus, Skull, Coins, Search, X, BookOpen, AlertTriangle } from 'lucide-react';
import { Monster, MonsterRank } from '../types';
import { calculateMonsterXp, calculateMonsterCredits } from '../constants';
...
export const MonsterManual: React.FC<MonsterManualProps> = ({ monsters, addMonster, onClose }) => {
  ...
  const generateMonster = async () => {
    if (!process.env.API_KEY) {
      alert("API Key not found in environment. Please check setup.");
      return;
    }
    ...
    try {
      ...
      // Prompt updated to request Rank
      const prompt = `Generate a unique LitRPG sci-fi monster in JSON format. 
      It should have: name, description (2 sentences max), level (2-20), rank (Trash, Regular, Champion, or Boss),
      stats (STR, PER, DEX, MEM, INT, CHA with values 1-10), and 2-3 abilities (names only).
      The output must be a valid JSON object. Do not wrap in markdown code blocks.`;
      ...
      if (text) {
        ...
        const rank: MonsterRank = data.rank || 'Regular';
        const level = data.level || 2;
        
        // Calculate XP based on rules
        const calculatedXp = calculateMonsterXp(level, rank);
        const calculatedCredits = calculateMonsterCredits(calculatedXp);

        const newMonster: Monster = {
          id: `gen-${Date.now()}`,
          name: data.name || "Unknown Horror",
          description: data.description || "Data corrupted.",
          level: level,
          rank: rank,
          xpReward: calculatedXp,
          credits: calculatedCredits,
          stats: data.stats || { STR:1, PER:1, DEX:1, MEM:1, INT:1, CHA:1 },
      ...
      alert("Failed to summon monster from the void.");
    } finally {
          ...
          <div className="w-full md:w-80 bg-slate-900/50 border-r border-slate-700 flex flex-col shrink-0 p-4 overflow-y-auto">
              ...
              <div className="bg-purple-900/10 border border-purple-500/30 p-4 rounded-xl mb-6">
                  <h3 className="text-purple-300 font-bold mb-2 flex items-center gap-2">
                      <Ghost size={16} /> New Entry
                  </h3>
                  <p className="text-xs text-slate-400 mb-4">
                      Scan the void for new biological or mechanical threats.
                  </p>
                  <button
                    onClick={generateMonster}
                    disabled={isGenerating}
                    className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-purple-600 hover:bg-purple-500 text-white rounded-lg text-sm font-bold transition-all disabled:opacity-50 shadow-lg shadow-purple-900/20"
                    >
                    {isGenerating ? <Loader2 className="animate-spin" size={16} /> : <Plus size={16} />}
                    <span>Generate Threat</span>
                    </button>
              </div>
                  ...
                  <div className="relative">
                   ...
                   <div>
                       ...
                       <div className="grid grid-cols-2 gap-2">
                           {['All', 'Trash', 'Regular', 'Champion', 'Boss'].map((r) => (
                               <button
                                  key={r}
                                  onClick={() => setFilterRank(r as any)}
                                ...
                                <div className="grid grid-cols-6 gap-1 mb-4">
                                    {Object.entries(monster.stats).map(([key, val]) => (
                                        <div key={key} className="bg-slate-950 rounded p-1 text-center border border-slate-800">
                                            ...
                                            <div className="text-xs font-bold text-slate-300">{val}</div>
                                        </div>
                                    ))}
                                </div>
