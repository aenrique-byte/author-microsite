...
import { ScrollText, Download, Wand2, Loader2, CheckCircle, XCircle, Circle } from 'lucide-react';
...
export const QuestSystem: React.FC<QuestSystemProps> = ({ character, quests, addQuest, updateQuestStatus }) => {
  ...
  const generateQuest = async () => {
    ...
    try {
      ...
      const prompt = `Create a LitRPG quest for a Level ${character.level} ${character.className} in JSON format.
      Fields: title, description, rewards (string text), steps (array of strings).
      Theme: Sci-fi, Cyberpunk, Dystopian.
      ...
      console.error(e);
      alert("Comms interference. Quest generation failed.");
    } finally {
    ...
    <div className="space-y-6">
      <div className="flex gap-2">
         <button 
           ...
           className="flex-1 flex items-center justify-center gap-2 bg-nexus-success/20 hover:bg-nexus-success/30 text-nexus-success border border-nexus-success/50 py-2 rounded-lg transition-all"
         >
           {isGenerating ? <Loader2 className="animate-spin" /> : <Wand2 size={18} />}
           <span>Accept Random Contract</span>
         </button>
         <button 
      ...
      <div className="space-y-4">
        {quests.length === 0 && (
          <div className="text-center text-slate-500 py-8 italic">
            No active contracts. Generation required.
          </div>
        )}
        {quests.map(quest => (
