...
// Helper: send admin notification when a new booking is created (no email to guest)
function sendNewBookingNotification($booking) {
    $adminEmail = defined('ADMIN_EMAIL') ? ADMIN_EMAIL : '';
    if (!$adminEmail) {
        return ['success' => false, 'log' => 'ADMIN_EMAIL not defined'];
    }

    $fromEmail = defined('FROM_EMAIL') ? FROM_EMAIL : 'noreply@example.com';
    $fromName = defined('FROM_NAME') ? FROM_NAME : 'Shoutout Manager';
...
<body style="font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, \'Helvetica Neue\', Arial, sans-serif; background: #f8fafc; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: #ffffff; border-radius: 12px; padding: 24px; box-shadow: 0 4px 10px rgba(15,23,42,0.08);">
    ...
    <p style="margin-top: 20px; font-size: 12px; color: #94a3b8;">To approve or reject this request, log into your Shoutout Manager admin panel and open the Requests tab.</p>
  </div>
</body>
</html>';
...
function sendApprovalEmail($pdo, $booking, $story, $shoutouts) {
    ...
    // Remove emoji from subject - some mail servers don't like it
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, \'Segoe UI\', Roboto, \'Helvetica Neue\', Arial, sans-serif; background-color: #f1f5f9;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
        ...
        <div style="background: white; padding: 30px; border-radius: 0 0 16px 16px; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
            ...
            <div style="margin-top: 30px; padding-top: 20px; border-top: 1px solid #e2e8f0; text-align: center;">
                ...
                <p style="color: #cbd5f5; font-size: 11px; margin: 8px 0 0 0;">
                    Don&#39;t want to receive these? Reply or email
                    <a href="mailto:noreply@ocwanderer.com?subject=Unsubscribe%20Shoutout%20Emails" style="color: #6366f1; text-decoration: underline;">noreply@ocwanderer.com</a>
    ...
    if (!$userSent) {
        ...
        $userSentMail = mail($to, $subject, $htmlBody, $headerString);
        $debug['mail_user'] = [
            'sent' => $userSentMail,
        ];
        $userSent = $userSentMail;
    }

    // Admin copy via SMTP (best effort)
    $adminSent = false;
    if ($adminEmail) {
        ...
        if (!$adminSent) {
            // Fallback to mail() for admin copy
            ...
            $adminSentMail = mail($adminEmail, $adminSubject, $htmlBody, $headerString);
            $debug['mail_admin'] = [
                'sent' => $adminSentMail,
            ];
            $adminSent = $adminSentMail;
        }
    }

    // Log email attempt for debugging (view at /shoutouts/api/email-log.txt)
    $logFile = __DIR__ . '/email-log.txt';
...
// GET /api/bookings.endpoint.php - Get all bookings or filtered by storyId
if ($method === 'GET') {
    ...
    $result = array_map(function($booking) {
        return [
            ...
            'createdAt' => strtotime($booking['created_at']) * 1000 // Convert to milliseconds
        ];
    }, $bookings);
...
// POST /api/bookings.endpoint.php - Create a new booking
if ($method === 'POST') {
    $input = getJsonInput();
    ...
    // Return the created booking (frontend format)
    $booking = [
        'id' => $id,
    ...
    // Send admin-only notification about the new request (no guest email here)
    try {
        sendNewBookingNotification($booking);
    } catch (Exception $e) {
        // Best-effort only; don't break the public booking flow
        $logFile = __DIR__ . '/email-log.txt';
...
// PUT /api/bookings.endpoint.php - Update booking status
if ($method === 'PUT') {
    $input = getJsonInput();
    ...
    if ($input['status'] === 'approved') {
        $emailDebug['status'] = 'approved';
        $emailDebug['booking_story_id'] = $booking['story_id'];
        ...
        $emailDebug['story_found'] = $story ? true : false;
        
        if ($story) {
            $emailDebug['story_title'] = $story['title'];
            $emailDebug['story_color'] = $story['color'] ?? 'not set';
            
            // Get shoutout codes for this story (or global ones)
            $stmt = $pdo->prepare("SELECT * FROM shoutout_admin_shoutouts WHERE story_id = ? OR story_id IS NULL OR story_id = ''");
            $stmt->execute([$booking['story_id']]);
            $shoutouts = $stmt->fetchAll();
            
            $emailDebug['shoutouts_count'] = count($shoutouts);
            $emailDebug['sending_to'] = $booking['email'];
            $emailDebug['admin_email'] = defined('ADMIN_EMAIL') ? ADMIN_EMAIL : 'not defined';
            
            // Send the email
            try {
                $emailResult = sendApprovalEmail($pdo, $booking, $story, $shoutouts);
                $emailDebug['email_result'] = $emailResult;
            } catch (Exception $e) {
                $emailDebug['email_error'] = $e->getMessage();
            }
        } else {
            $emailDebug['error'] = 'Story not found in database';
        }
    }
...
// DELETE /api/bookings.endpoint.php?id=xxx - Delete booking
if ($method === 'DELETE') {
    $id = $_GET['id'] ?? null;
