...
export default function PublicPage({ onAdminLogin }: PublicPageProps) {
  ...
  // Background image setup - using author profile backgrounds with fallback
  const backgroundImage = currentTheme === 'light'
    ? getRandomBackground(
        authorProfile?.background_image_light,
        '/images/lofi_light_bg.webp'
      )
    : getRandomBackground(
        authorProfile?.background_image_dark,
        '/images/lofi_bg.webp'
      );
  const overlayClass = 'bg-black bg-opacity-20'; // 20% opacity dark overlay as requested
  ...
  const loadInitialData = async () => {
    try {
      const [configData, storiesData, profileResponse] = await Promise.all([
        db.getConfig(),
        db.getStories(),
        fetch('/api/author/get.php').then(res => res.json())
      ]);

      setConfig(configData);
      setStories(storiesData);

      // Set author profile for background images
  ...
  if (stories.length === 0) {
    return (
      <div className="min-h-screen bg-neutral-100 dark:bg-neutral-900 flex items-center justify-center p-4">
        <div className="max-w-md text-center bg-white dark:bg-neutral-800 p-8 rounded-xl shadow-lg">
          <div className="text-6xl mb-4">📚</div>
          <h2 className="text-2xl font-bold mb-4">No Stories Configured</h2>
          <p className="text-neutral-600 dark:text-neutral-400 mb-6">
            The shoutout system needs at least one story to be configured by the admin.
          </p>
          {onAdminLogin && (
            <button
  ...
  return (
    <div className="relative min-h-screen font-sans text-neutral-800 dark:text-neutral-100 transition-colors duration-200">
      <PageNavbar breadcrumbs={[{ label: 'Shoutouts' }]} />
      
      {/* Fixed background image layer */}
      <div
        className="fixed inset-0 z-0 bg-no-repeat bg-top [background-size:auto_100%] md:[background-size:100%_auto]"
        style={{
          backgroundImage: `url('${backgroundImage}')`,
          backgroundColor: currentTheme === 'light' ? '#f7f7f7' : '#0a0a0a',
        }}
      />
      ...
      {/* Hero Section */}
      <div className="relative z-10 text-white pb-20 pt-8 px-4 shadow-lg border-b border-neutral-800">
        ...
        <div className="max-w-5xl mx-auto">
            ...
            <div className="flex flex-col md:flex-row items-center gap-8">
                <div className="w-48 h-72 flex-shrink-0 rounded-lg overflow-hidden shadow-2xl border-4 border-neutral-700/50 transform -rotate-2 hover:rotate-0 transition-transform duration-500">
      ...
      <main className="relative z-10 max-w-5xl mx-auto -mt-10 px-4 pb-20">
        <div className="grid lg:grid-cols-3 gap-8">
            {/* Calendar Column */}
            <div className="lg:col-span-2">
                <div className="flex justify-between items-center mb-4 bg-white dark:bg-neutral-800/50 p-2 rounded-lg backdrop-blur-sm border border-neutral-200 dark:border-neutral-700/50">
                    <button 
                    onClick={() => setCurrentMonthIndex(i => Math.max(0, i - 1))}
                    disabled={currentMonthIndex <= 0}
                    className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-neutral-100 dark:hover:bg-neutral-700 disabled:opacity-50 disabled:cursor-not-allowed text-neutral-700 dark:text-neutral-200 font-bold text-sm"
                    >
                    <ChevronLeft size={18} /> Prev
                    </button>
                    <span className="text-xs text-neutral-500 dark:text-neutral-400 font-black uppercase tracking-widest">
                    Available Months
                    </span>
                    <button 
            ...
            <div className="lg:col-span-1">
                <div className="sticky top-8 space-y-6">
                    ...
                    {selectedDate && !bookingSuccess && (
                        <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-xl border border-neutral-200 dark:border-neutral-700 overflow-hidden">
                            ...
                            <form onSubmit={handleSubmit} className="p-6 space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase mb-1">Author / Discord Name</label>
                                    <input 
                                        required
                                        value={authorName}
                                        onChange={e => setAuthorName(e.target.value)}
                                        className="w-full p-2 border border-neutral-300 dark:border-neutral-600 bg-white dark:bg-neutral-700 text-neutral-900 dark:text-white rounded focus:ring-2 focus:ring-amber-500 outline-none"
                                        placeholder="e.g. RRAngel"
                                    />
                                </div>
                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase mb-1">Email (for confirmation)</label>
                                    <input 
                                        required
                                        type="email"
                                        value={email}
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase mb-1">Story Link</label>
                                    <input 
                                        required
                                        type="url"
                                        value={storyLink}
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase mb-1">Shoutout Code (HTML)</label>
                                    <textarea 
                                        required
                                        value={shoutoutCode}
                                        onChange={e => setShoutoutCode(e.target.value)}
                                        className="w-full h-32 p-2 text-xs font-mono bg-neutral-50 dark:bg-neutral-900 dark:text-neutral-300 border border-neutral-300 dark:border-neutral-600 rounded focus:ring-2 focus:ring-amber-500 outline-none"
                                ...
                                {shoutoutCode && (
                                    <div className="mt-2">
                                        <label className="block text-xs font-bold text-neutral-500 dark:text-neutral-400 uppercase mb-1">Preview</label>
                                        <div className="scale-75 origin-top-left border border-neutral-200 dark:border-neutral-600 rounded p-2 bg-white dark:bg-neutral-800">
                                ...
                                <div className="pt-4 border-t border-neutral-100 dark:border-neutral-700">
                                    <label className="flex items-center gap-3 cursor-pointer p-3 bg-neutral-50 dark:bg-neutral-700/50 rounded border border-neutral-200 dark:border-neutral-600 hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-colors">
                                        <div className={`w-5 h-5 border-2 rounded flex items-center justify-center ${antiBotChecked ? `${theme.bg} ${theme.border}` : 'border-neutral-300 dark:border-neutral-500'}`}>
                                        ...
                                        <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300">I am human (Anti-bot)</span>
                                    </label>
                                </div>
                    ...
                    {/* Success State */}
                    {bookingSuccess && (
                        <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-xl border border-green-200 dark:border-green-800 overflow-hidden">
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <p className="text-sm font-semibold text-neutral-700 dark:text-neutral-200 border-t pt-4 dark:border-neutral-700">
                                ...
                                {adminShoutouts.map(s => (
                                    <div key={s.id} className="space-y-2">
                                        <div className="flex justify-between items-center">
                                            <span className="text-xs font-bold uppercase text-neutral-500 dark:text-neutral-400">{s.label}</span>
                                        ...
                                        <div className="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 rounded p-2 text-xs font-mono text-neutral-500 dark:text-neutral-400 h-20 overflow-y-auto whitespace-pre-wrap select-all">
                                            {s.code}
                                        </div>
                                    </div>
                                ))}

                                <button onClick={() => { setSelectedDate(null); setBookingSuccess(null); }} className="w-full py-2 border border-neutral-300 dark:border-neutral-600 text-neutral-600 dark:text-neutral-300 font-bold rounded hover:bg-neutral-50 dark:hover:bg-neutral-700">
                                    Close
                                </button>
                            </div>
                        </div>
                    ...
                    {!selectedDate && !bookingSuccess && (
                         <div className="bg-white dark:bg-neutral-800 p-6 rounded-xl shadow-sm border border-neutral-200 dark:border-neutral-700">
                             <h3 className="font-bold text-lg mb-4 text-neutral-800 dark:text-neutral-100">How it works</h3>
                             <ul className="space-y-4 text-sm text-neutral-700 dark:text-neutral-200">
                                 <li className="flex gap-3">
                                     <span className={`${theme.bg} text-white font-bold w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 border ${theme.border}`}>1</span>
                                     <span>Find a <span className={`${theme.text} font-bold`}>colored</span> date on the calendar.</span>
                                 </li>
                                 <li className="flex gap-3">
                                     <span className={`${theme.bg} text-white font-bold w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 border ${theme.border}`}>2</span>
                                     <span>Request the slot with your story details.</span>
                                 </li>
                                 <li className="flex gap-3">
                                     <span className={`${theme.bg} text-white font-bold w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 border ${theme.border}`}>3</span>
                                     <span>Wait for approval (you'll get an email). Grab my code while you wait!</span>
                                 </li>
                             </ul>

                             <div className="mt-8 pt-8 border-t border-neutral-100 dark:border-neutral-700 text-center">
                                 <p className="text-xs text-neutral-500 dark:text-neutral-400 mb-2">Owner of this site?</p>
                                 <button onClick={onAdminLogin} className="text-xs font-bold text-neutral-600 dark:text-neutral-300 hover:text-neutral-800 dark:hover:text-neutral-100 flex items-center justify-center gap-1 mx-auto">
