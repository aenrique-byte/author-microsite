...
import { getAllContracts } from '../contracts-constants';
                    ...
                    <div className="flex justify-between items-start mb-3">
                      <div>
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          <span className={`text-xs uppercase font-bold ${TYPE_COLORS[contract.contract_type]}`}>
                            {contract.contract_type}
                          </span>
                          <span className={`text-xs px-2 py-0.5 rounded border uppercase font-bold ${DIFFICULTY_COLORS[contract.difficulty]}`}>
                            {contract.difficulty}
                          </span>
                      ...
                      <div className="space-y-2 mb-4 bg-slate-900/50 p-4 rounded-lg">
                        ...
                        {contract.objectives.map((obj, i) => (
                          <div key={i} className="flex items-start gap-2 text-sm text-slate-300">
                            <span className={`mt-0.5 ${obj.current >= obj.target ? 'text-green-400' : 'text-slate-600'}`}>
                              {obj.current >= obj.target ? <CheckCircle size={14} /> : <Circle size={14} />}
                            </span>
                    ...
                    {contract.rewards && (
                      <div className="flex flex-wrap gap-3 text-sm">
                        <div className="flex items-center gap-1 bg-nexus-accent/10 text-nexus-accent px-3 py-1.5 rounded border border-nexus-accent/20">
                          <Zap size={14} />
                          <span className="font-bold">{contract.rewards.xp?.toLocaleString()} XP</span>
                        </div>
                        <div className="flex items-center gap-1 bg-yellow-500/10 text-yellow-400 px-3 py-1.5 rounded border border-yellow-500/20">
                          <Coins size={14} />
                          <span className="font-bold">{contract.rewards.credits?.toLocaleString()} Credits</span>
                        </div>
                        {contract.rewards.items && contract.rewards.items.length > 0 && (
