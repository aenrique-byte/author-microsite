...
export const BattleSimulator: React.FC<BattleSimulatorProps> = ({ character, monsters, onApplyResult, currentDbClass }) => {
  ...
  const calculate = () => {
    ...
    if (mode === 'combat') {
        ...
        const primaryAttr = getPrimaryAttribute();
        const secondaryAttr = getSecondaryAttribute();
        const primaryAttrVal = character.attributes[primaryAttr];
        const secondaryAttrVal = character.attributes[secondaryAttr];

        // Formula: Total monster XP * (1 + Primary/100 + Secondary/100 * 0.6)
        // Example: If primary=20, secondary=15: 1 + 0.2 + 0.09 = 1.29x XP
